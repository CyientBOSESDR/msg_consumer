...
PREPARE_LOGGING(msg_consumer_i)

void msg_consumer_i::messageReceived(const std::string &id, const Component_Members_struct &msg)
{
	    LOG_INFO(msg_consumer_i, id<<" "<<msg.Float<<" "<<msg.String);
	    std::cout<< msg.Float<<" "<<msg.String;

}


msg_consumer_i::msg_consumer_i(const char *uuid, const char *label) :
    msg_consumer_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

msg_consumer_i::~msg_consumer_i()
{
}

void msg_consumer_i::constructor()
{
    /***********************************************************************************
     This is the RH constructor. All properties are properly initialized before this function is called 
    ***********************************************************************************/

	message_in->registerMessage("Component_Members", this,&msg_consumer_i::messageReceived);
}
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void msg_consumer_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &msg_consumer_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &msg_consumer_i::<callback method>)
        in the constructor.
        ...
        //Add to msg_consumer.cpp
        msg_consumer_i::msg_consumer_i(const char *uuid, const char *label) :
            msg_consumer_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &msg_consumer_i::scaleChanged);
            addPropertyListener(status, this, &msg_consumer_i::statusChanged);
        }

        void msg_consumer_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void msg_consumer_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            RH_DEBUG(this->_baseLog, "status changed");
        }
            
        //Add to msg_consumer.h
...
************************************************************************************************/
int msg_consumer_i::serviceFunction()
{
